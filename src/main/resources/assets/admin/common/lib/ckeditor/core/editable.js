...
(function () {
    ...
    CKEDITOR.editable = CKEDITOR.tools.createClass({
        ...
        proto: {
            focus: function () {
                ...
                try {
                    if (CKEDITOR.env.ie && !( CKEDITOR.env.edge && CKEDITOR.env.version > 14 ) &&
            ...
            setup: function () {
                ...
                if (styles) {
                    ...
                    if (!stylesElement) {
                        ...
                        CKEDITOR.env.ie && CKEDITOR.env.version < 9 ? stylesElement.$.styleSheet.cssText = styles : stylesElement.setText(
                            styles);
                    }
                }
    ...
    CKEDITOR.editor.prototype.editable = function (element) {
        ...
        if (arguments.length) {
            editable = this._.editable = element
                ? ( element instanceof CKEDITOR.editable ? element : new CKEDITOR.editable(this, element) )
                ...
                if (helpLabel) {
                    ...
                    if (ct) {
                        var ariaDescId = CKEDITOR.tools.getNextId(),
                            desc = CKEDITOR.dom.element.createFromHtml('<span id="' + ariaDescId + '" class="cke_voice_label">' +
                                                                       helpLabel + '</span>');
                        ct.append(desc);
    ...
    function autoParagraphTag(editor) {
        return ( editor.activeEnterMode != CKEDITOR.ENTER_BR && editor.config.autoParagraph !== false )
        ...
        function insertDataIntoRange(that) {
            var range = that.range,
                ...
                separateEndContainer = !!endContainer.getCommonAncestor(startContainer) && // endC is not detached.
                                       pos != CKEDITOR.POSITION_IDENTICAL &&
                                       !( pos & CKEDITOR.POSITION_CONTAINS + CKEDITOR.POSITION_IS_CONTAINED ); // endC & endS are in separate branches.
            ...
            for (; nodeIndex < nodesData.length; nodeIndex++) {
                ...
                // Auto paragraphing.
                if (!nodeData.isBlock && shouldAutoParagraph(that.editor, path.block, path.blockLimit) &&
                    ( fixBlock = autoParagraphTag(that.editor) )) {
    ...
    getHtmlFromRangeHelpers = {
        ...
        cell: {
            ...
            shrink: function (that) {
                ...
                if (startContainer.type == CKEDITOR.NODE_ELEMENT && startContainer.equals(endContainer) && startContainer.is('tr') &&
    ...
    extractHtmlFromRangeHelpers = (function () {
        ...
        var table = (function () {
            ...
            function checkDisjointNodes(node1, node2) {
                ...
                return pos === CKEDITOR.POSITION_IDENTICAL ? false : ( ( pos & disallowedPositions ) === 0 );
            }
            ...
            return {
                ...
                detectPurge: function (that) {
                    ...
                    if (editablesCount > 1) {
                        ...
                        if (startTable && endTable && range.checkBoundaryOfElement(startTable, CKEDITOR.START) &&
                            range.checkBoundaryOfElement(endTable, CKEDITOR.END)) {
