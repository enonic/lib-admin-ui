...
(function () {
    var functions = [],
        cssVendorPrefix =
            CKEDITOR.env.gecko ? '-moz-' : CKEDITOR.env.webkit ? '-webkit-' : CKEDITOR.env.ie ? '-ms-' : '',
        ampRegex = /&/g,
        gtRegex = />/g,
        ltRegex = /</g,
        quoteRegex = /"/g,
    ...
    CKEDITOR.tools = {
        ...
        cssStyleToDomStyle: (function () {
            ...
            var cssFloat = ( typeof test.cssFloat != 'undefined' )
                ? 'cssFloat'
        ...
        fixDomain: function () {
            ...
            while (1) {
                try {
                    ...
                    domain = domain ? // If it is not the first pass, strip one part of the
                        // name. E.g.  "test.example.com"  => "example.com"
                             domain.replace(/.+?(?:\.|$)/, '') : // In the first pass, we'll handle the
