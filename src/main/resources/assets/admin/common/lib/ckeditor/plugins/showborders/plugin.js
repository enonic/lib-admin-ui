    ...
    CKEDITOR.plugins.add('showborders', {
        ...
        init: function (editor) {
            ...
            editor.on('removeFormatCleanup', function (evt) {
                ...
                if (editor.getCommand('showborders').state == CKEDITOR.TRISTATE_ON && element.is('table') &&
                    ( !element.hasAttribute('border') || parseInt(element.getAttribute('border'), 10) <= 0 )) {
                ...
                htmlFilter.addRules({
                    elements: {
                        'table': function (table) {
                            ...
                            cssClass && ( attributes['class'] =
                                cssClass.replace(showBorderClassName, '').replace(/\s{2}/, ' ').replace(/^\s+|\s+$/, '') );
                        }
                    }
                });
            }
