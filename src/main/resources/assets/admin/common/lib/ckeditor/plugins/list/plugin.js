...
(function () {
        ...
        arrayToList: function (listArray, database, baseIndex, paragraphMode, dir) {
            ...
            while (1) {
                ...
                if (item.indent == indentLevel) {
                    ...
                    var needsBlock = currentListItem.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT &&
                                     ( paragraphMode != CKEDITOR.ENTER_BR || dirLoose || style || className );
        ...
        exec: function (editor) {
            ...
            if (this.state == CKEDITOR.TRISTATE_OFF) {
                ...
                if (!editable.getFirst(nonEmpty)) {
                    config.enterMode == CKEDITOR.ENTER_BR ? editable.appendBogus() : ranges[0].fixBlock(1,
    ...
    function isTextBlock(node) {
        return node.type == CKEDITOR.NODE_ELEMENT &&
               ( node.getName() in CKEDITOR.dtd.$block || node.getName() in CKEDITOR.dtd.$listItem ) && CKEDITOR.dtd[node.getName()]['#'];
    }
