...
CKEDITOR.plugins.add('scayt', {
    ...
    init: function (editor) {
        ...
        editor.ui.add('Scayt', CKEDITOR.UI_MENUBUTTON, {
            ...
            onMenu: function () {
                ...
                editor.getMenuItem('scaytToggle').label =
                    editor.lang.scayt[(scaytInstance ? plugin.state.scayt[editor.name] : false) ? 'btn_disable' : 'btn_enable'];
    ...
    parseConfig: function (editor) {
        ...
        if (editor.config.scayt_maxSuggestions === undefined || typeof editor.config.scayt_maxSuggestions != 'number' ||
            editor.config.scayt_maxSuggestions < 0) {
            editor.config.scayt_maxSuggestions = 5;
        }

        if (editor.config.scayt_minWordLength === undefined || typeof editor.config.scayt_minWordLength != 'number' ||
            editor.config.scayt_minWordLength < 1) {
        ...
        if (editor.config.scayt_disableOptionsStorage) {
            var userOptions = CKEDITOR.tools.isArray(editor.config.scayt_disableOptionsStorage)
                ? editor.config.scayt_disableOptionsStorage
                : ( typeof editor.config.scayt_disableOptionsStorage === 'string' )
                                  ? [editor.config.scayt_disableOptionsStorage]
                                  : undefined,
                availableValue = ['all', 'options', 'lang', 'ignore-all-caps-words', 'ignore-domain-names', 'ignore-words-with-mixed-cases',
                    'ignore-words-with-numbers'],
                valuesOption = ['lang', 'ignore-all-caps-words', 'ignore-domain-names', 'ignore-words-with-mixed-cases',
                    'ignore-words-with-numbers'],
            ...
            var makeOptionsToStorage = function (options) {
                ...
                for (var i = 0; i < options.length; i++) {
                    ...
                    if (!isValidOption(value) || isGroupOptionInUserOptions && !!search(valuesOption, function (val) {
                            if (val === 'lang') {
                                return false;
                            }
                        })) {
        ...
        var plugin = CKEDITOR.plugins.scayt,
            ...
            pathFilter = function (element) {
                ...
                if (scaytInstance && (element.hasAttribute(plugin.options.data_attribute_name) ||
                                      element.hasAttribute(plugin.options.problem_grammar_data_attribute))) {
                    return false;
                }
            },
            ...
            var dataFilterRules = {
                elements: {
                    span: function (element) {
                        ...
                        var scaytState = element.hasClass(plugin.options.misspelled_word_class) &&
                                         element.attributes[plugin.options.data_attribute_name],
                            graytState = element.hasClass(plugin.options.problem_grammar_class) &&
                                         element.attributes[plugin.options.problem_grammar_data_attribute];
            ...
            if (isScaytNode) {
                ...
                for (var i = 0; i < suggestions.length; i++) {
                    ...
                    editor.addCommand(commandName,
                        self.createCommand(CKEDITOR.plugins.scayt.suggestions[i], replaceKeyName, updateEventName));
    ...
    menuGenerator: function (editor, selectionNode) {
        ...
        for (var key in menuItems) {
            if (isScaytNode && CKEDITOR.tools.indexOf(allowedOption, key.replace('scayt_', '')) == -1 &&
                editor.config.scayt_contextCommands != 'all') {
                continue;
            }

            if (isGrammarNode && key !== 'grayt_problemdescription' &&
                CKEDITOR.tools.indexOf(allowedOption, key.replace('grayt_', '')) == -1 && editor.config.scayt_contextCommands != 'all') {
                continue;
            }
    ...
    if (CKEDITOR.config.scayt_handleUndoRedo === true) {
        ...
        undoImagePrototype[equalsContentMethodName] = CKEDITOR.tools.override(undoImagePrototype[equalsContentMethodName], function (org) {
            return function (otherImage) {
                var pluginState = CKEDITOR.plugins.scayt && CKEDITOR.plugins.scayt.state.scayt[otherImage.editor.name] &&
                                  otherImage.editor.scayt,
