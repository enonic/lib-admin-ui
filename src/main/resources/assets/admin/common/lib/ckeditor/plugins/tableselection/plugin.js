...
( function () {
    ...
    TableSelection.prototype.insertRow = function (count, insertBefore, clearSelection) {
        ...
        for (var i = 0; i < count; i++) {
            ...
            newCells = CKEDITOR.tools.array.filter(row.find('td, th').toArray(), function (cell) {
                return clearSelection ? true : cell.$.cellIndex >= cellIndexFirst && cell.$.cellIndex <= cellIndexLast;
            });
    ...
    function pasteListener(evt) {
        ...
        editor.once('afterPaste', function () {
            var toSelect = pastedTableMap ? getCellsBetween(new CKEDITOR.dom.element(pastedTableMap[0][0]),
                new CKEDITOR.dom.element(getLastArrayItem(getLastArrayItem(pastedTableMap)))) : tableSel.cells.all;
