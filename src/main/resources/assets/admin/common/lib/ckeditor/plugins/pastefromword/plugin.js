...
(function () {
    ...
    CKEDITOR.plugins.add('pastefromword', {
        ...
        init: function (editor) {
            ...
            var forceFromWord = 0,
                path = this.path,
                configInlineImages = editor.config.pasteFromWord_inlineImages === undefined
                    ? true
                    : editor.config.pasteFromWord_inlineImages;
            ...
            editor.on('paste', function (evt) {
                var data = evt.data,
                    dataTransferHtml = CKEDITOR.plugins.clipboard.isCustomDataTypesSupported
                        ? data.dataTransfer.getData('text/html', true)
                        : null,
                    // Required in Paste from Word Image plugin (#662).
                    dataTransferRtf = CKEDITOR.plugins.clipboard.isCustomDataTypesSupported ? data.dataTransfer.getData('text/rtf') : null,
            ...
            function imagePastingListener(evt) {
                ...
                function createSrcWithBase64(img) {
                    return img.type ? 'data:' + img.type + ';base64,' +
                                      CKEDITOR.tools.convertBytesToBase64(CKEDITOR.tools.convertHexStringToBytes(img.hex)) : null;
                }
