        ...
        init: function (editor) {
            ...
            editor.on('doubleclick', function (evt) {
                ...
                if (element && !element.isReadOnly()) {
                    if (element.is('a')) {
                        evt.data.dialog = ( element.getAttribute('name') && ( !element.getAttribute('href') || !element.getChildCount() ) )
                            ? 'anchor'
                            : 'link';
    ...
    CKEDITOR.removeAnchorCommand.prototype = {
        exec: function (editor) {
            ...
            if (sel && ( anchor = sel.getSelectedElement() ) &&
                ( !anchor.getChildCount() ? CKEDITOR.plugins.link.tryRestoreFakeAnchor(editor, anchor) : anchor.is('a') )) {
                anchor.remove(1);
            } else {
