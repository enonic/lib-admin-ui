...
(function () {
    ...
    function applyInlineStyle(range) {
        ...
        while (currentNode) {
            ...
            if (currentNode.equals(lastNode)) {
                ...
                if (checkIfNodeCanBeChildOfStyle(def, currentNode, lastNode, nodeName, dtd, nodeIsNoStyle, nodeIsReadonly,
                        includeReadonly)) {
        ...
        if (range.collapsed) {
            ...
            for (var i = 0, element; i < startPath.elements.length && ( element = startPath.elements[i] ); i++) {
                ...
                if (this.checkElementRemovable(element)) {
                    ...
                    if (!alwaysRemoveElement && range.collapsed && ( range.checkBoundaryOfElement(element, CKEDITOR.END) || ( isStart =
                            range.checkBoundaryOfElement(element, CKEDITOR.START) ) )) {
    ...
    function removeBlockStyle(range) {
        ...
        while (( block = iterator.getNextParagraph() )) {
            if (this.checkElementRemovable(block)) {
                ...
                if (block.is('pre')) {
                    newBlock = this._.enterMode == CKEDITOR.ENTER_BR ? null : range.document.createElement(
    ...
    function mergePre(preBlock) {
        var previousBlock;
        if (!( ( previousBlock = preBlock.getPrevious(nonWhitespaces) ) && previousBlock.type == CKEDITOR.NODE_ELEMENT &&
    ...
    function removeFromElement(element, keepDataAttrs) {
        ...
        for (var attName in attributes) {
            ...
            if (( attName == 'class' || this._.definition.fullMatch ) &&
        ...
        if (attributes) {
            for (var i = 0; i < attributes.length; i++) {
                ...
                if (( actualAttrValue = element.getAttribute(attName) )) {
                    ...
                    //    - The override definition value is a regex that
                    //      has matches in the attribute value.
                    if (attValue === null || ( attValue.test && attValue.test(actualAttrValue) ) ||
