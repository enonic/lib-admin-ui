...
(function () {
    ...
    function updateCollapsed(range) {
        range.collapsed =
            ( range.startContainer && range.endContainer && range.startContainer.equals(range.endContainer) && range.startOffset ==
              range.endOffset );
    }
    ...
    CKEDITOR.dom.range.prototype = {
                ...
                while (enlargeable || sibling) {
                    ...
                    while (sibling) {
                        ...
                        if (sibling.type == CKEDITOR.NODE_COMMENT) {
                            ...
                            if (( sibling.$.offsetWidth > offsetWidth0 || excludeBrs && sibling.is('br') ) &&
        ...
        shrink: function (mode, selectContents, options) {
            var shrinkOnBlockBoundary = typeof options === 'boolean' ? options : ( options &&
                                                                                   typeof options.shrinkOnBlockBoundary === 'boolean'
            ...
            if (!this.collapsed) {
                ...
                if (moveStart) {
                    ...
                    textStart &&
                    this.setStartAt(textStart, selectContents ? CKEDITOR.POSITION_AFTER_START : CKEDITOR.POSITION_BEFORE_START);
                }
        ...
        _find: function (query, includeNonEditables) {
            ...
            if (ancestor && ancestor.find) {
                ...
                for (i = 0; i < initialMatches.count(); i++) {
                    ...
                    isStartGood =
                        ( curItem.getPosition(boundaries.startNode) & CKEDITOR.POSITION_FOLLOWING ) || boundaries.startNode.equals(curItem);
                    isEndGood =
                        ( curItem.getPosition(boundaries.endNode) & ( CKEDITOR.POSITION_PRECEDING + CKEDITOR.POSITION_IS_CONTAINED ) ) ||
                        boundaries.endNode.equals(curItem);
