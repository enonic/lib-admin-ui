...
(function () {
    ...
    function initLine(that) {
        var doc = that.doc,
            ...
            line = newElementFromHtml('<span contenteditable="false" style="' + CSS_COMMON + 'position:absolute;border-top:1px dashed ' +
                                      that.boxColor + '"></span>', doc),
            iconPath = CKEDITOR.getUrl(this.path + 'images/' + ( env.hidpi ? 'hidpi/' : '' ) + 'icon' + ( that.rtl ? '-rtl' : '' ) +
                                       '.png');
        ...
        extend(line, {
            ...
            place: function () {
                ...
                if (parent.equals(editable)) {
                    ...
                    styleSet.left = any.size.left - any.size.margin.left + view.scroll.x -
    ...
    function triggerEditable(that) {
        ...
        // \->	Bottom case.
            ...
            triggerLook = that.inInlineMode ||
                          inBetween(edgeNode.size.bottom, view.pane.height - triggerOffset, view.pane.height) ? LOOK_BOTTOM : LOOK_NORMAL;
    ...
    function triggerEdge(that) {
        ...
        if (!elementSibling) {
            ...
            triggerSetup = [null, element][bottomTrigger ? 'reverse' : 'concat']().concat([
                ...
                TYPE_EDGE,
                triggerLook,
                element.equals(that.editable[bottomTrigger ? 'getLast' : 'getFirst'](that.isRelevant)) ? ( bottomTrigger
                    ? LOOK_BOTTOM
    ...
    function getSize(that, element, ignoreScroll, force) {
        ...
        box.top = docPosition.y - ( ignoreScroll ? 0 : that.view.scroll.y ), box.left =
            docPosition.x - ( ignoreScroll ? 0 : that.view.scroll.x ),

            // w/ borders and paddings.
            box.outerWidth = element.$.offsetWidth, box.outerHeight = element.$.offsetHeight,

            // w/o borders and paddings.
            box.height = box.outerHeight - ( padding.top + padding.bottom + border.top + border.bottom ), box.width =
            box.outerWidth - ( padding.left + padding.right + border.left + border.right ),
