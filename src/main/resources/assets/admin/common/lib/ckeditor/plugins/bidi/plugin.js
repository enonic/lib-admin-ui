...
(function () {
    ...
    function setToolbarStates(editor, path) {
        ...
        var selectionDir = useComputedState ? selectedElement.getComputedStyle('direction') : selectedElement.getStyle('direction') ||
                                                                                              selectedElement.getAttribute('dir');
    ...
    function switchDir(element, dir, editor, database) {
        ...
        var elementDir = useComputedState ? element.getComputedStyle('direction') : element.getStyle('direction') ||
    ...
    function getFullySelected(range, elements, enterMode) {
        ...
        if (range.checkBoundaryOfElement(ancestor, CKEDITOR.START) && range.checkBoundaryOfElement(ancestor, CKEDITOR.END)) {
            var parent;
            while (ancestor && ancestor.type == CKEDITOR.NODE_ELEMENT && ( parent = ancestor.getParent() ) && parent.getChildCount() == 1 &&
                ...
                if (ranges && ranges.length) {
                    ...
                    while (( range = rangeIterator.getNextRange(1) )) {
                        ...
                        if (!selectedElement || selectedElement &&
                                                !( selectedElement.type == CKEDITOR.NODE_ELEMENT && selectedElement.getName() in
                                                directSelectionGuardElements )) {
                        ...
                        walker.evaluator = function (node) {
                            ...
                            return !!( isNodeElement(node) && isGuard(node) &&
                                       !( node.is(enterTagName) && isNodeElement(node.getParent()) && node.getParent().is('blockquote') ) &&
                                       ...
                                       ( ( node.getPosition(end) & CKEDITOR.POSITION_PRECEDING + CKEDITOR.POSITION_CONTAINS ) ==
    ...
    function dirChangeNotifier(org) {
        ...
        return function (name, val) {
            if (!this.isReadOnly()) {
                var orgDir;
                if (( name == ( isAttribute || isRemoveAttribute ? 'dir' : 'direction' ) || name == 'style' &&
                      ( isRemoveAttribute || dirStyleRegexp.test(val) ) ) && !isOffline(this)) {
